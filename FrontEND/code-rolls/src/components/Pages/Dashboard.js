...
function Dashboard() {
  ...
  async function getDashboardData() {
    const response = await axios.post(
      "http://localhost:3000/objectives/checkstreak",
      {
        user_id: "6436005c3c4616f372641688",
      }
    );
    const {
      user: { initialStreakDay, perfectDays, nutriPoints, objectives },
      currentDay,
    } = response.data;
  ...
  return (
    <>
    <br></br>
      <h3 style={{ marginLeft: "10px" }} >Mi progreso</h3>
      <div className="d-flex p-5">
        ...
        <Card>
          ...
          />
          <Streak
            title="Toma al menos 2L de agua"
            buttonText="Listo!"
          ...
          />
          <Streak
            title="Haz al menos 20min de actividad física"
            buttonText="Listo!"
            containerColor="silver"
            valores={obj}
            streak={2}
        ...
        <Card>
          <div className="d-flex justify-content-center">
            <NutriPoints
              title={["NutriPoints", "Días perfectos"]}
              containerColor="silver"
              points={points}
              perfectDays={perf}
              animar={[animando1, animando2]}
            />
          </div>
